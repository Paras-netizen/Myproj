...
int main()
{
	...
	for(i=0;i<n;i++)
	{
		if(priority[i]>=0 && priority[i]<=5) //roundrobin
		{
			...
			t1+=burstt[i];
			j++;
			printf("\n P[%d] belongs to Queue 1(Round Robin)",i+1);
		}
		...
		if(priority[i]>=6 && priority[i]<=10) //priority
		{
			...
			t2+=burstt[i];
			k++;
			printf("\n P[%d] belongs to Queue 2(Priority Sched)",i+1);
		}
		...
		if(priority[i]>=11 && priority[i]<=15) //fcfs
		{
			...
			t3+=burstt[i];
			l++;
			printf("\n P[%d] belongs to Queue 3(First Come First Serve)",i+1);
		}
	}
	...
	while(t1+t2+t3!=0)
	{
		...
		{
			t1-=10;
			printf("\n Queue1 [RR] will run for 10 units  Left Burst time = %d units\n\t CPU Shift to Next Queue\n",t1);
		}
		else if(t1>0)
		{
			printf("\n Queue1 [RR] will run for %d units  Left Burst time = 0 units\n\t CPU Shift to Next Queue\n" ,t1);
			t1=0;
		}
		//Priority
		if(t2>=10)
		{
			printf("\n Queue2 [PRIORITY] will run for 10 units  Left Burst time = %d units\n\t CPU Shift to Next Queue\n",t2);
			t2-=10;
		}
		else if(t2>0)
		{
			printf("\n Queue2 [PRIORITY]  will run for %d units  Left Burst time = 0 units\n\t CPU Shift to Next Queue\n" ,t2);
			t2=0;
		}
		//FCFS
		if(t3>=10)
		{
			t3-=10;
			printf("\n Queue3 [FCFS] will run for 10 units  Left Burst time = %d units\n\t CPU Shift to Next Queue\n",t3);
		}
		else if(t3>0)
		{
			printf("\n Queue3 [FCFS] will run for %d units  Left Burst time = 0 units\n\t CPU Shift to Next Queue\n" ,t3);
			t3=0; 
		}
	}
	r_robin();
	...
	{
		int total=0,i=0;
		int m=0,temp=0,pos=0,min=0,p=0;
		...
		for(i=0;i<l;i++)
		{
			if(total!=0 && BT3[i]>(10-total))
			{
				printf("\n [Queue3] P[%d] Will Run For %d Units",i+7,10-total);
				BT3[i]=BT3[i]-(10-total);
				total=0;
				printf("\n\n Quantum Time = 10 Finished for [Q3]\n\t CPU Shift to Next Queue\n");
				r_robin();
				break;
			}
			else if(BT3[i]>0 && BT3[i]<=10)
			{
				printf("\n [Queue3] P[%d] Will Run For %d Units",i+7,BT3[i]);
				total+=BT3[i];
				BT3[i]=0;
			}
		}
